...
function handle_ajax_password_lost() {
    ...
    if (empty($_POST['user_login'])) {
		...
		wp_send_json_error(['message' => clu_render_message('email_required')]);
        exit;
    }
    ...
    if (!$user_data) {
		...
		wp_send_json_error(['message' => clu_render_message('user_not_found')]);
        exit;
    }
    ...
    if (is_wp_error($reset_key)) {
		...
		wp_send_json_error(['message' => clu_render_message('general_error')]);
        exit;
    }
    ...
    if (!$email_sent) {
		...
		wp_send_json_error(['message' => clu_render_message('reset_email_send_failed')]);
        exit;
    }

    // Respond with success
    // wp_send_json_success(['message' => 'Password reset email has been sent. Please check your inbox.']);
    wp_send_json_success(['message' => clu_render_message('reset_email_sent')]);
